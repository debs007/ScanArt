...
struct RemoteTouchEvent: Codable {
    ...
    let kind: Kind
    let normalizedX: Double   // 0…1 relative to broadcaster screen width
    let normalizedY: Double   // 0…1 relative to broadcaster screen height
}

/// Central state machine for the peer-to-peer broadcast/control session.
/// One instance lives in `RemoteControlHubView` and is shared with child views.
@Observable
final class RemoteControlSession: NSObject {
    ...
    /// Increments on every incoming touch; use onChange(of:) to react.
    ...
    func session(_ session: MCSession, peer peerID: MCPeerID, didChange state: MCSessionState) {
        DispatchQueue.main.async {
            ...
            case .notConnected:
                ...
                // Controller re-browses after disconnect
...
extension RemoteControlSession: MCNearbyServiceAdvertiserDelegate {
    func advertiser(_ advertiser: MCNearbyServiceAdvertiser, didReceiveInvitationFromPeer peerID: MCPeerID, withContext context: Data?, invitationHandler: @escaping (Bool, MCSession?) -> Void) {
        // Accept only when no controller is connected yet (one controller per broadcaster)
